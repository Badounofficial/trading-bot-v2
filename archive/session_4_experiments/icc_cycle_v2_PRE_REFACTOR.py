...
def _compute_initial_tp(
    ...
    """
    Take Profit computation (per ICC spec):
        Primary: closest opposite-direction OB on H4 or Daily, in favorable direction
        Fallback: measured move = entry + 3 * |entry - SL|  (RR 1:3 per user preference)
    
    For BUY: look for OB- (bearish supply) ABOVE the entry price
    For SELL: look for OB+ (bullish demand) BELOW the entry price
    
    "Active" OB = not consumed at the time of entry (h1_bar).
    We pick the CLOSEST favorable OB to maximize hit probability.
    """
    risk = abs(setup.entry_price - setup.sl_current)
    
    # Try to find opposite OB on H4 first, then Daily
    candidate_obs: list[OrderBlock] = []
    
    target_type = 'OB-' if setup.direction == Direction.BUY else 'OB+'
    
    # Collect candidate OBs (not yet consumed in real-time terms)
    for ob_list in (h4_obs or [], daily_obs or []):
        for ob in ob_list:
            if ob.type != target_type:
                continue
            # Must be the right side of entry price
            if setup.direction == Direction.BUY:
                # Need OB- above entry (sell zone above current price)
                ...
                if ob.zone_high >= setup.entry_price:
                    continue
            # Must not be consumed yet at entry time
            # (we use h4_ob.detected_at_bar timing as a rough proxy)
            # For Daily: ob.detected_at_bar is in Daily frame, harder to compare
            # For simplicity v1: accept all OBs that are not currently consumed
            if ob.consumed:
                continue
            candidate_obs.append(ob)
    
    if candidate_obs:
        # Pick the closest in favorable direction
        if setup.direction == Direction.BUY:
            # Want the LOWEST OB- (closest above entry)
            best = min(candidate_obs, key=lambda ob: ob.zone_low)
            # TP at the lower edge of the OB- zone (where price first meets the OB)
            tp = best.zone_low
            source = "OB_OPPOSITE"
        else:
            # Want the HIGHEST OB+ (closest below entry)
            best = max(candidate_obs, key=lambda ob: ob.zone_high)
            # TP at the upper edge of the OB+ zone
            tp = best.zone_high
            source = "OB_OPPOSITE"
        
        # Sanity: if TP is too close (less than 1:1 RR), fallback to measured move
        tp_distance = abs(tp - setup.entry_price)
        if tp_distance < risk * 1.0:
            # OB too close to be useful, use measured move instead
            if setup.direction == Direction.BUY:
                tp = setup.entry_price + 3 * risk
            else:
                tp = setup.entry_price - 3 * risk
            source = "MEASURED_MOVE_1to3"
        return tp, source
    
    # Fallback: measured move 1:3 (user choice for wider TP)
    if setup.direction == Direction.BUY:
        tp = setup.entry_price + 3 * risk
    else:
        tp = setup.entry_price - 3 * risk
    return tp, "MEASURED_MOVE_1to3"
...
def run_icc_cycle(
    ...
    Args:
        asset: name of the asset (for tracking)
        daily_prices, h4_prices, h1_prices: OHLCV DataFrames per TF
        mode: trading mode (SWING/INTRADAY/SCALPING) — SWING for now
        *_lookback: swing_lookback for each TF
    
    Returns:
        List of all TradeSetup objects that went through their lifecycle.
    """
    ...
    for h1_bar in range(len(h1_prices)):
        ...
        # Compute Daily bias at this point in time
