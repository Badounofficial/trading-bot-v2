# Rollback procedure — Snapshot 20260627T010744Z (label: pre_phase2_to_phase3_transition_backup)
...
git reset --hard $(cat snapshots/SNAPSHOT_20260627T010744Z_pre_phase2_to_phase3_transition_backup/git_head.txt)

# If uncommitted changes were captured in a stash, reapply
STASH_SHA=$(cat snapshots/SNAPSHOT_20260627T010744Z_pre_phase2_to_phase3_transition_backup/git_stash_sha.txt)
[ -n "$STASH_SHA" ] && git stash apply "$STASH_SHA"
```

### Step 3 — Restore daemon state files
```bash
cp snapshots/SNAPSHOT_20260627T010744Z_pre_phase2_to_phase3_transition_backup/daemon_state.json live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260627T010744Z_pre_phase2_to_phase3_transition_backup/heartbeat.txt    live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260627T010744Z_pre_phase2_to_phase3_transition_backup/trades.jsonl     live/state/ 2>/dev/null || true
```
