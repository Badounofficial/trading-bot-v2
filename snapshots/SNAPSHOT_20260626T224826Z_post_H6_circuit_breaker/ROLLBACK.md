# Rollback procedure — Snapshot 20260626T224826Z (label: post_H6_circuit_breaker)
...
git reset --hard $(cat snapshots/SNAPSHOT_20260626T224826Z_post_H6_circuit_breaker/git_head.txt)

# If uncommitted changes were captured in a stash, reapply
STASH_SHA=$(cat snapshots/SNAPSHOT_20260626T224826Z_post_H6_circuit_breaker/git_stash_sha.txt)
[ -n "$STASH_SHA" ] && git stash apply "$STASH_SHA"
```

### Step 3 — Restore daemon state files
```bash
cp snapshots/SNAPSHOT_20260626T224826Z_post_H6_circuit_breaker/daemon_state.json live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260626T224826Z_post_H6_circuit_breaker/heartbeat.txt    live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260626T224826Z_post_H6_circuit_breaker/trades.jsonl     live/state/ 2>/dev/null || true
```
