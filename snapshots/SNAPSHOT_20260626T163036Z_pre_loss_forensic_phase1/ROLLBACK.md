# Rollback procedure — Snapshot 20260626T163036Z (label: pre_loss_forensic_phase1)
...
git reset --hard $(cat snapshots/SNAPSHOT_20260626T163036Z_pre_loss_forensic_phase1/git_head.txt)

# If uncommitted changes were captured in a stash, reapply
STASH_SHA=$(cat snapshots/SNAPSHOT_20260626T163036Z_pre_loss_forensic_phase1/git_stash_sha.txt)
[ -n "$STASH_SHA" ] && git stash apply "$STASH_SHA"
```

### Step 3 — Restore daemon state files
```bash
cp snapshots/SNAPSHOT_20260626T163036Z_pre_loss_forensic_phase1/daemon_state.json live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260626T163036Z_pre_loss_forensic_phase1/heartbeat.txt    live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260626T163036Z_pre_loss_forensic_phase1/trades.jsonl     live/state/ 2>/dev/null || true
```
