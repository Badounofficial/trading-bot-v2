# Rollback procedure — Snapshot 20260625T174725Z (label: pre_cutover_25juin_mac_dead)
...
git reset --hard $(cat snapshots/SNAPSHOT_20260625T174725Z_pre_cutover_25juin_mac_dead/git_head.txt)

# If uncommitted changes were captured in a stash, reapply
STASH_SHA=$(cat snapshots/SNAPSHOT_20260625T174725Z_pre_cutover_25juin_mac_dead/git_stash_sha.txt)
[ -n "$STASH_SHA" ] && git stash apply "$STASH_SHA"
```

### Step 3 — Restore daemon state files
```bash
cp snapshots/SNAPSHOT_20260625T174725Z_pre_cutover_25juin_mac_dead/daemon_state.json live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260625T174725Z_pre_cutover_25juin_mac_dead/heartbeat.txt    live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260625T174725Z_pre_cutover_25juin_mac_dead/trades.jsonl     live/state/ 2>/dev/null || true
```
