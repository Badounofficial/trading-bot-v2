# Rollback procedure — Snapshot 20260626T171422Z (label: post_H1_min_hold_extended)
...
git reset --hard $(cat snapshots/SNAPSHOT_20260626T171422Z_post_H1_min_hold_extended/git_head.txt)

# If uncommitted changes were captured in a stash, reapply
STASH_SHA=$(cat snapshots/SNAPSHOT_20260626T171422Z_post_H1_min_hold_extended/git_stash_sha.txt)
[ -n "$STASH_SHA" ] && git stash apply "$STASH_SHA"
```

### Step 3 — Restore daemon state files
```bash
cp snapshots/SNAPSHOT_20260626T171422Z_post_H1_min_hold_extended/daemon_state.json live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260626T171422Z_post_H1_min_hold_extended/heartbeat.txt    live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260626T171422Z_post_H1_min_hold_extended/trades.jsonl     live/state/ 2>/dev/null || true
```
