# Rollback procedure — Snapshot 20260622T003348Z (label: post-phase-b-fix-applied)
...
git reset --hard $(cat snapshots/SNAPSHOT_20260622T003348Z_post-phase-b-fix-applied/git_head.txt)

# If uncommitted changes were captured in a stash, reapply
STASH_SHA=$(cat snapshots/SNAPSHOT_20260622T003348Z_post-phase-b-fix-applied/git_stash_sha.txt)
[ -n "$STASH_SHA" ] && git stash apply "$STASH_SHA"
```

### Step 3 — Restore daemon state files
```bash
cp snapshots/SNAPSHOT_20260622T003348Z_post-phase-b-fix-applied/daemon_state.json live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260622T003348Z_post-phase-b-fix-applied/heartbeat.txt    live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260622T003348Z_post-phase-b-fix-applied/trades.jsonl     live/state/ 2>/dev/null || true
```
