# Rollback procedure — Snapshot 20260626T232553Z (label: post_H2_entry_threshold)
...
git reset --hard $(cat snapshots/SNAPSHOT_20260626T232553Z_post_H2_entry_threshold/git_head.txt)

# If uncommitted changes were captured in a stash, reapply
STASH_SHA=$(cat snapshots/SNAPSHOT_20260626T232553Z_post_H2_entry_threshold/git_stash_sha.txt)
[ -n "$STASH_SHA" ] && git stash apply "$STASH_SHA"
```

### Step 3 — Restore daemon state files
```bash
cp snapshots/SNAPSHOT_20260626T232553Z_post_H2_entry_threshold/daemon_state.json live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260626T232553Z_post_H2_entry_threshold/heartbeat.txt    live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260626T232553Z_post_H2_entry_threshold/trades.jsonl     live/state/ 2>/dev/null || true
```
