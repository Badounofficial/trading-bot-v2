# Rollback procedure — Snapshot 20260622T002253Z (label: pre-phase-b-baseline)
...
git reset --hard $(cat snapshots/SNAPSHOT_20260622T002253Z_pre-phase-b-baseline/git_head.txt)

# If uncommitted changes were captured in a stash, reapply
STASH_SHA=$(cat snapshots/SNAPSHOT_20260622T002253Z_pre-phase-b-baseline/git_stash_sha.txt)
[ -n "$STASH_SHA" ] && git stash apply "$STASH_SHA"
```

### Step 3 — Restore daemon state files
```bash
cp snapshots/SNAPSHOT_20260622T002253Z_pre-phase-b-baseline/daemon_state.json live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260622T002253Z_pre-phase-b-baseline/heartbeat.txt    live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260622T002253Z_pre-phase-b-baseline/trades.jsonl     live/state/ 2>/dev/null || true
```
