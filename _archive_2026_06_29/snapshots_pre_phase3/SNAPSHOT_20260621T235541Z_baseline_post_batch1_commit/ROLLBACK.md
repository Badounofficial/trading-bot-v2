# Rollback procedure — Snapshot 20260621T235541Z (label: baseline_post_batch1_commit)
...
git reset --hard $(cat snapshots/SNAPSHOT_20260621T235541Z_baseline_post_batch1_commit/git_head.txt)

# If uncommitted changes were captured in a stash, reapply
STASH_SHA=$(cat snapshots/SNAPSHOT_20260621T235541Z_baseline_post_batch1_commit/git_stash_sha.txt)
[ -n "$STASH_SHA" ] && git stash apply "$STASH_SHA"
```

### Step 3 — Restore daemon state files
```bash
cp snapshots/SNAPSHOT_20260621T235541Z_baseline_post_batch1_commit/daemon_state.json live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260621T235541Z_baseline_post_batch1_commit/heartbeat.txt    live/state/ 2>/dev/null || true
cp snapshots/SNAPSHOT_20260621T235541Z_baseline_post_batch1_commit/trades.jsonl     live/state/ 2>/dev/null || true
```
